...
import TodoApiServices from '../services/TodoApiServices';
...
class Main extends Component {
  ...
  // CDM (Promise)
  componentDidMount() {
    TodoApiServices.todoServiceList()
      .then((response) => {
        this.setState({
          todos: response.data,
        }); // end setState
  ...
  todoCreateSubmit = async (event) => {
    ...
    TodoApiServices.todoServiceCreate(todoDto)
      .then((response) => {
  ...
  // DELETE
  todoDeleteSubmit(id) {
    TodoApiServices.todoServiceDeleteById(id)
      .then(
        () => {
          this.setState({
            todos: this.state.todos.filter((todos) => todos.id != id), //end filter
          }); // end setState
        } // end response
      )
      .catch((error) => {
  ...
  // DELETE ALL
  todoDeleteAll() {
    TodoApiServices.todoServiceDeleteAll()
  ...
  todoDeleteDoneAll() {
    this.state.todos.map((todo) => {
      todo.completed
        ? this.todoDeleteSubmit(todo.id)
        : console.log('silinmeyenler');
    });
    window.location.reload();
  }
  ...
  onChangeIsDone = (event) => {
    ...
    todos.map((todo) => {
      if (event.target.id == todo.id) {
        ...
        TodoApiServices.todoServiceCreate(todo)
          .then((response) => {
            if (response.status == 200) {
  ...
  render() {
    ...
    return (
      <div className="container">
        <div className="row">
          <h3 className="text-center">Görevlerim</h3>
          <div className="col-6 offset-3 border pt-3 pb-3">
            {/* NEW INPUT ADD */}
            <Input
            ...
            <div className="list ">
              <div className="row text-center">
                <h3 className="col-12 mt-4">Görev Listesi</h3>
                {/* <div className="col">
                  <button
                    onClick={() => {
                      this.doneView(1);
